...
export default function TicketDetails() {
  ...
  const ticketIdRaw = Array.isArray(params.ticketId)
    ? params.ticketId[0]
    : params.ticketId;
  ...
  useEffect(() => {
    ...
    getTicketById(ticketId).then(setTicket).catch(console.error);
  }, [ticketId]);
...
useEffect(() => {
  ...
  // --- RETURN (only UI is conditional, NOT hooks) ---
  return (
    <div className="min-h-screen flex flex-col items-center bg-gradient-to-br from-cyan-300 to-sky-600 p-8 pt-24">
      <div className="w-full max-w-4xl bg-white shadow-md rounded-xl p-8 flex flex-col gap-6 max-h-[80vh] overflow-y-auto">
        ...
        {/* Display invalid ID */}
        {isNaN(ticketId) && <p>Invalid Ticket ID</p>}

        {/* Display loading */}
        {!isNaN(ticketId) && !ticket && <p>Loading ticket...</p>}

        {/* Only show UI when ticket is loaded */}
        {ticket && (
          <>
